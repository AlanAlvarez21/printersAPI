...
# Local record count file configuration
...
# Use different state files for different environments to avoid conflicts
if os.name == 'nt':  # Windows
    STATE_FILE = os.path.join(application_path, "dbf_state_corrected_windows.json")
    LAST_MODIFIED_FILE = os.path.join(application_path, "last_modified_state_windows.json")
else:  # Unix/Linux/Mac
    STATE_FILE = os.path.join(application_path, "dbf_state_corrected_unix.json")
    LAST_MODIFIED_FILE = os.path.join(application_path, "last_modified_state_unix.json")
...
class CorrectedSchemaUploader:
    def __init__(self):
        ...
        # Initialize local record count tracking
        ...
        try:
            if os.path.exists(RECORD_COUNT_FILE):
                with open(RECORD_COUNT_FILE, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"Loaded previous record count: {count}")
                    return count
        except Exception as e:
            logger.warning(f"Could not load previous record count file: {e}")
        return 0
    ...
    def save_previous_record_count(self, count: int) -> bool:
        ...
        try:
            ...
            logger.info(f"Saved new record count: {count}")
            return True
        except Exception as e:
            logger.error(f"Error saving previous record count: {e}")
            return False
    ...
    def get_dbf_record_count(self) -> int:
        ...
        try:
            ...
            logger.info(f"DBF file has {record_count} records")
            return record_count
        except Exception as e:
            logger.error(f"Failed to get record count from DBF file: {e}")
            return -1
    ...
    def records_changed(self) -> bool:
        ...
        try:
            ...
            if current_dbf_count == -1:
                logger.warning("Could not get DBF record count, assuming changes exist")
                return True
            
            logger.info(f"Comparing records: DBF has {current_dbf_count}, previously had {self.previous_record_count}")
            ...
            if current_dbf_count > self.previous_record_count:
                logger.info(f"New records detected: {current_dbf_count - self.previous_record_count} new records to process")
                return True
            elif current_dbf_count < self.previous_record_count:
                logger.warning(f"DBF has fewer records than before: {self.previous_record_count - current_dbf_count} difference - this may indicate an issue")
                # In this case, we may want to re-sync everything
                # For now, let's return True to process all records
                return True
            else:
                logger.info("No new records detected")
                return False
        except Exception as e:
            logger.error(f"Error checking if records changed: {e}")
    ...
    def generate_record_hash(self, record: Dict) -> str:
        ...
        for key, value in record.items():
            ...
            if hasattr(value, 'strftime'):  # This will catch date, datetime, etc.
                ...
                # Handle special cases for consistent string representation
                if value is None:
                    record_copy[key] = ""
                elif isinstance(value, (int, float)):
                    record_copy[key] = str(value)
                else:
                    record_copy[key] = str(value).strip()
        
        # Create a sorted tuple of key-value pairs for consistent hashing
        record_items = sorted(record_copy.items())
        record_str = json.dumps(record_items, sort_keys=True, ensure_ascii=False, separators=(',', ':'))
    ...
    def has_record_changed(self, no_opro: str, record: Dict) -> bool:
        ...
        if record_hash != last_hash:
            logger.debug(f"Record {no_opro} has changed")
            logger.debug(f"Old hash: {last_hash}")
            logger.debug(f"New hash: {record_hash}")
    ...
    def process_dbf_file(self) -> bool:
        ...
        try:
            ...
            # Check if there are new records by comparing DBF and database counts
            if not self.records_changed():
                logger.info("No new records detected, skipping upload")
            ...
            # Debug information
            logger.debug(f"Total records in DBF: {len(dbf)}")
...
def main():
    """Main function"""
    logger.info("Starting CORRECTED SCHEMA DBF Uploader with Database Comparison")
    ...
    while True:
        try:
            logger.info("Checking for DBF file updates and record changes...")
            if force_send:

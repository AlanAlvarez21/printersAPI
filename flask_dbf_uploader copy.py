    ...
    def process_dbf_file(self) -> bool:
        ...
        try:
            ...
            for record in dbf:
                ...
                # On first run, process all records
                # On subsequent runs, only process changed records
                if self.first_run or self.has_record_changed(no_opro, record_dict):
            ...
            # Debug information
            logger.debug(f"Total records in DBF: {processed_count + (len(dbf) - processed_count)}")
            logger.debug(f"Records with changes: {changed_records_count}")
